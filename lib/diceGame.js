...
class DiceGame {
  ...
  play() {
    ...
    const randomFlip = rollDie(2).number;
    ...
    if (userFirst) {
      ...
      compDie = rest[rollDie(rest.length).number]; 
    } else {
      const compChoice = rollDie(this.diceArr.length).number;
  ...
  _roll(die, label) {
    ...
    const compSecret = rollDie(6).number; 
